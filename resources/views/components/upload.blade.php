<div class="pb-6 h-full">
    ...
    <div class="mb-4 p-4 bg-white rounded-md shadow-custom">
        ...
        <div class="mt-3 flex items-center gap-3 p-3 bg-gray-50 rounded-md">
            <label class="text-sm text-gray-600 whitespace-nowrap">压缩质量</label>
            <input type="range" id="upload-quality" min="10" max="100" step="5" value="100" class="flex-1">
            <span id="upload-quality-val" class="text-sm text-gray-600 w-10 text-right">100</span>
...
@push('scripts')
    <script>
        ...
        const qualitySlider = document.getElementById('upload-quality');
        const qualityVal = document.getElementById('upload-quality-val');
    ...
    <script>
        ...
        $picker.fileupload({
            ...
            formData: (form) => {
                return [
                    {name: 'strategy_id', value: $('#strategy-selected').data('id')},
                    {name: 'quality', value: $('#upload-quality').val() || 100}
                ];
            },
            paste: (e, data) => {
                let files = [];
