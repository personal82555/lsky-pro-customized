...
class ImageService
{
    ...
    public function store(Request $request): Image
    {
        ...
        if (! is_null($user)) {
            ...
            // 图片保存至指定相册或默认相册
            if ($albumIdParam = $request->input('album_id')) {
                if ($user->albums()->where('id', $albumIdParam)->exists()) {
                    $image->album_id = $albumIdParam;
                }
        ...
        if (! in_array($extension, ['ico', 'gif'])) {
            // 图片保存质量与格式，优先使用上传时指定的质量
            $quality = (int) $request->input('quality', $configs->get(GroupConfigKey::ImageSaveQuality, 100));
            ...
            if ($quality < 100 || $format) {
                ...
                $handleImage = $manager->make($file)->save($format, $quality);
